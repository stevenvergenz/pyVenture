...
from game.main import main as launchGame
...
def buildMenu():
	...
	for name in glob.glob('*.pvm') + glob.glob('*.pvm.gz'):
		feature = types.Feature( name, 'A potential named %1'.format(name) )
		feature.parentArea = world.areas[1]
		world.areas[1].features.append(feature)

		action = types.Action( 'Embody potential %1'.format(name), 'A new universe spins off, and you are caught in the middle of it!' )
		action.parentFeature = feature
		feature.actions.append(action)

		event = LoadWorldEvent({'filename': name})
